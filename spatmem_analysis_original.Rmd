...
```{r}
colSums(is.na(dataset_SpatMem_4))
```

# Str and table of studyGroup
```{r}
str(dataset_SpatMem_4)

dataset_SpatMem_4$targetCollected <- as.numeric(dataset_SpatMem_4$targetCollected)
dataset_SpatMem_4$straightness.index <- as.numeric(dataset_SpatMem_4$straightness.index)
dataset_SpatMem_4$changepoints <- as.numeric(dataset_SpatMem_4$changepoints)
dataset_SpatMem_4$trialDuration <- as.numeric(dataset_SpatMem_4$trialDuration)
dataset_SpatMem_4$interactions <- as.numeric(dataset_SpatMem_4$interactions)
dataset_SpatMem_4$trial_count <- as.numeric(dataset_SpatMem_4$trial_count)

summary(dataset_SpatMem_4)

table(dataset_SpatMem_4$studyGroup)
```

# Distribution of targetCollected/straightness.index/changepoints/trialDuration/interactions/studyGroup
```{r}
par(mfrow = c(3,2))
barplot(table(dataset_SpatMem_4$targetCollected),main="Distribution of targetCollected",xlab="targetCollected",ylab="Frequency")
hist(dataset_SpatMem_4$straightness.index, breaks=50,main="Distribution of variable straightness.index",xlab="Number of straightness.index",ylab="Frequency")
hist(dataset_SpatMem_4$changepoints, breaks=15,main="Distribution of variable changepoints",xlab="Number of changepoints",ylab="Frequency")
hist(dataset_SpatMem_4$trialDuration, breaks=50,main="Distribution of variable trialDuration",xlab="Number of votes for trialDuration",ylab="Frequency")
hist(dataset_SpatMem_4$interactions, breaks=50,main="Distribution of variable interactions",xlab="Number of votes for interactions",ylab="Frequency")
barplot(table(dataset_SpatMem_4$studyGroup),main="Distribution of studyGroup",xlab="studyGroup",ylab="Frequency")
```

# Boxplot of changepoints across each studyGroup 
```{r}
boxplot(changepoints~studyGroup, data = dataset_SpatMem_4, xlab="Study Group", ylab="Change Points", col = c("gold", "darkgreen"), cex.axis = 0.8)
```

# Hist facet
```{r}
agg <- aggregate(changepoints ~ studyGroup, dataset_SpatMem_4, function(x) {
...
ggplot(data = dataset_SpatMem_4, aes(x = changepoints, fill=studyGroup)) + 
...
ggplot(dataset_SpatMem_4, aes(x = changepoints)) +
...
```{r}
fit1 <- lm(straightness.index~changepoints, data = dataset_SpatMem_4)
summary(fit1)
tab_model(fit1)

fit2 <- lm(trialDuration~changepoints, data = dataset_SpatMem_4)
summary(fit2)
tab_model(fit2)
...
```{r}
t.test(dataset_SpatMem_4$straightness.index, dataset_SpatMem_4$changepoints)
t.test(dataset_SpatMem_4$trialDuration, dataset_SpatMem_4$changepoints)
```

# Summary Table
```{r}
summary(lm(changepoints~factor(studyGroup),data=dataset_SpatMem_4))
tab_model(lm(changepoints~factor(studyGroup),data=dataset_SpatMem_4),
  string.pred = "Coeffcient",
  string.ci = "95% CI",
  string.p = "P-Value")

tab_model(lm(straightness.index~factor(studyGroup),data=dataset_SpatMem_4),
          lm(trialDuration~factor(studyGroup),data=dataset_SpatMem_4),
  string.pred = "Coeffcient",
  string.ci = "95% CI",
  string.p = "P-Value")
```
...
```{r}
fit2=lm(straightness.index~factor(studyGroup)*changepoints,data=dataset_SpatMem_4)
summary(fit2)

fit3=lm(trialDuration~factor(studyGroup)*changepoints,data=dataset_SpatMem_4)
summary(fit3)
...
```{r}
fib.lm <- lm(straightness.index ~ changepoints*factor(studyGroup), data = dataset_SpatMem_4)
...
```{r}
fib.lm <- lm(trialDuration ~ changepoints*factor(studyGroup), data = dataset_SpatMem_4)
fib.rg <- ref_grid(fib.lm, at = list(changepoints = c(0, 5, 6.368, 13)))
emmip(fib.rg, factor(studyGroup)~changepoints, CI=TRUE) 
```
...
```{r}
ggplot(dataset_SpatMem_4, aes(x = studyGroup, y = changepoints)) +
geom_jitter() +
scale_x_discrete() +
...
ggplot(dataset_SpatMem_4, aes(x = reorder(studyGroup, -changepoints, mean), y = changepoints)) + 
geom_jitter() +
scale_x_discrete() +
theme(axis.text.x = element_text(angle = 90)) + 
labs(x = "studyGroup", y = "changepoints")
...
```{r}
z = subset(dataset_SpatMem_4, targetCollected == 1, select = logFileName:trial_count)
ggplot(dataset_SpatMem_4, aes(x = reorder(studyGroup, -changepoints, mean), y = changepoints, color = as.factor(targetCollected))) +
geom_jitter() +
scale_x_discrete() +
...
```{r}
ggplot(dataset_SpatMem_4, aes(x = straightness.index, y = changepoints, colour = studyGroup)) + 
...
```{r}
ggplot(dataset_SpatMem_4, aes(x = trialDuration, y = changepoints, colour = studyGroup)) + 
geom_line(aes(group = targetCollected), linewidth = 1) + 
scale_color_brewer(palette = "Set2") +
facet_wrap(~ studyGroup, ncol=2) +
...
ggplot(dataset_SpatMem_4, aes(x = straightness.index, y = changepoints, colour = as.factor(targetCollected))) + 
geom_point() +
scale_color_brewer(palette = "Set2") +
...
ggplot(dataset_SpatMem_4, aes(x = trialDuration, y = changepoints, colour = as.factor(targetCollected))) + 
geom_point() +
scale_color_brewer(palette = "Set2") +
facet_wrap(~ studyGroup, ncol=6) + 
theme_minimal() + 
...
```{r}
cor_dataset_SpatMem <- dplyr::select(dataset_SpatMem_4, changepoints, straightness.index, trialDuration, interactions, trial_count, studyGroup)
...
```{r}
ggplot(dataset_SpatMem_4, aes(changepoints, straightness.index)) + 
...
# Boxplot facet
```{r}
ggplot(dataset_SpatMem_4, aes(x=straightness.index, y=changepoints, group=studyGroup)) + 
  geom_boxplot(aes(fill=studyGroup)) +
  facet_grid(targetCollected~studyGroup, margins=TRUE)

ggplot(dataset_SpatMem_4, aes(x=trialDuration, y=changepoints, group=studyGroup)) + 
  geom_boxplot(aes(fill=studyGroup)) +
  facet_grid(targetCollected~studyGroup, margins=TRUE) +
...
```{r}
SpatMem_4 <- dplyr::select(dataset_SpatMem_4, targetCollected, changepoints, straightness.index, trialDuration, interactions, trial_count, studyGroup)
  
# Prepare the data
SpatMem_4_pca <- SpatMem_4[,c("targetCollected","changepoints","interactions","trial_count","straightness.index","trialDuration")]
...
# ???specific participant/studyGroup
```{r}
SpatMem_4 <- dplyr::select(dataset_SpatMem_4, participant, targetCollected, changepoints, straightness.index, trialDuration, interactions, trial_count, studyGroup)
...
plot(hclust_result, cex = 0.1)
```

```{r}
cluster_df <- cutree(hclust_result, k = 8) %>% 
  tibble(participant = names(.), cluster = as.factor(.))

ggplot(trips %>% inner_join(cluster_df, by = "participant"), aes(x, y, col = cluster)) + 
  geom_jitter(size = 0.6, alpha = 0.1) + 
  scale_color_brewer(palette = "Set2") + 
  labs(x = "Longitude", y = "Latitude") + 
  guides(color = guide_legend("Cluster", override.aes = list(size = 2, alpha = 1))) + 
  theme(legend.position = "bottom")
```
