...
```{r}
colSums(is.na(dataset_SpatMem_1))
```

# Str and table of studyGroup
```{r}
str(dataset_SpatMem_1)

dataset_SpatMem_1$targetCollected <- as.numeric(dataset_SpatMem_1$targetCollected)
dataset_SpatMem_1$straightness.index <- as.numeric(dataset_SpatMem_1$straightness.index)
dataset_SpatMem_1$changepoints <- as.numeric(dataset_SpatMem_1$changepoints)
dataset_SpatMem_1$trialDuration <- as.numeric(dataset_SpatMem_1$trialDuration)
dataset_SpatMem_1$interactions <- as.numeric(dataset_SpatMem_1$interactions)
dataset_SpatMem_1$trial_count <- as.numeric(dataset_SpatMem_1$trial_count)

summary(dataset_SpatMem_1)

table(dataset_SpatMem_1$studyGroup)
```

# Distribution of targetCollected/straightness.index/changepoints/trialDuration/interactions/studyGroup
```{r}
par(mfrow = c(3,2))
barplot(table(dataset_SpatMem_1$targetCollected),main="Distribution of targetCollected",xlab="targetCollected",ylab="Frequency")
hist(dataset_SpatMem_1$straightness.index, breaks=50,main="Distribution of variable straightness.index",xlab="Number of straightness.index",ylab="Frequency")
hist(dataset_SpatMem_1$changepoints, breaks=15,main="Distribution of variable changepoints",xlab="Number of changepoints",ylab="Frequency")
hist(dataset_SpatMem_1$trialDuration, breaks=50,main="Distribution of variable trialDuration",xlab="Number of votes for trialDuration",ylab="Frequency")
hist(dataset_SpatMem_1$interactions, breaks=50,main="Distribution of variable interactions",xlab="Number of votes for interactions",ylab="Frequency")
barplot(table(dataset_SpatMem_1$studyGroup),main="Distribution of studyGroup",xlab="studyGroup",ylab="Frequency")
```

# Boxplot of changepoints across each studyGroup 
```{r}
boxplot(changepoints~studyGroup, data = dataset_SpatMem_1, xlab="Study Group", ylab="Change Points", col = c("gold", "darkgreen"), cex.axis = 0.8)
```

# Hist facet
```{r}
agg <- aggregate(changepoints ~ studyGroup, dataset_SpatMem_1, function(x) {
...
ggplot(data = dataset_SpatMem_1, aes(x = changepoints, fill=studyGroup)) + 
...
ggplot(dataset_SpatMem_1, aes(x = changepoints)) +
...
```{r}
fit1 <- lm(straightness.index~changepoints, data = dataset_SpatMem_1)
summary(fit1)
tab_model(fit1)

fit2 <- lm(trialDuration~changepoints, data = dataset_SpatMem_1)
summary(fit2)
tab_model(fit2)
...
```{r}
t.test(dataset_SpatMem_1$straightness.index, dataset_SpatMem_1$changepoints)
#t.test(dataset_SpatMem_1$trialDuration, dataset_SpatMem_1$changepoints)
```

# Summary Table
```{r}
summary(lm(changepoints~factor(studyGroup),data=dataset_SpatMem_1))
tab_model(lm(changepoints~factor(studyGroup),data=dataset_SpatMem_1),
  string.pred = "Coeffcient",
  string.ci = "95% CI",
  string.p = "P-Value")

#tab_model(lm(straightness.index~factor(studyGroup),data=dataset_SpatMem_1),
#          lm(trialDuration~factor(studyGroup),data=dataset_SpatMem_1),
#  string.pred = "Coeffcient",
#  string.ci = "95% CI",
#  string.p = "P-Value")
```
...
```{r}
fit2=lm(straightness.index~factor(studyGroup)*changepoints,data=dataset_SpatMem_1)
summary(fit2)

fit3=lm(trialDuration~factor(studyGroup)*changepoints,data=dataset_SpatMem_1)
summary(fit3)
...
```{r}
fib.lm <- lm(straightness.index ~ changepoints*factor(studyGroup), data = dataset_SpatMem_1)
...
```{r}
fib.lm <- lm(trialDuration ~ changepoints*factor(studyGroup), data = dataset_SpatMem_1)
fib.rg <- ref_grid(fib.lm, at = list(changepoints = c(0, 5, 6.368, 13)))
emmip(fib.rg, factor(studyGroup)~changepoints, CI=TRUE) 
```
...
```{r}
ggplot(dataset_SpatMem_1, aes(x = studyGroup, y = changepoints)) +
geom_jitter() +
scale_x_discrete() +
...
ggplot(dataset_SpatMem_1, aes(x = reorder(studyGroup, -changepoints, mean), y = changepoints)) + 
geom_jitter() +
scale_x_discrete() +
theme(axis.text.x = element_text(angle = 90)) + 
labs(x = "studyGroup", y = "changepoints")
...
```{r}
z = subset(dataset_SpatMem_1, targetCollected == 1, select = logFileName:trial_count)
ggplot(dataset_SpatMem_1, aes(x = reorder(studyGroup, -changepoints, mean), y = changepoints, color = as.factor(targetCollected))) +
geom_jitter() +
scale_x_discrete() +
...
```{r}
ggplot(dataset_SpatMem_1, aes(x = straightness.index, y = changepoints, colour = studyGroup)) + 
...
```{r}
ggplot(dataset_SpatMem_1, aes(x = trialDuration, y = changepoints, colour = studyGroup)) + 
geom_line(aes(group = targetCollected), linewidth = 1) + 
scale_color_brewer(palette = "Set2") +
facet_wrap(~ studyGroup, ncol=2) +
...
ggplot(dataset_SpatMem_1, aes(x = straightness.index, y = changepoints, colour = as.factor(targetCollected))) + 
geom_point() +
scale_color_brewer(palette = "Set2") +
...
ggplot(dataset_SpatMem_1, aes(x = trialDuration, y = changepoints, colour = as.factor(targetCollected))) + 
geom_point() +
scale_color_brewer(palette = "Set2") +
facet_wrap(~ studyGroup, ncol=6) + 
theme_minimal() + 
...
```{r}
cor_dataset_SpatMem <- dplyr::select(dataset_SpatMem_1, changepoints, straightness.index, trialDuration, interactions, trial_count, studyGroup)
...
```{r}
ggplot(dataset_SpatMem_1, aes(changepoints, straightness.index)) + 
...
# Boxplot facet
```{r}
ggplot(dataset_SpatMem_1, aes(x=straightness.index, y=changepoints, group=studyGroup)) + 
  geom_boxplot(aes(fill=studyGroup)) +
  facet_grid(targetCollected~studyGroup, margins=TRUE)

ggplot(dataset_SpatMem_1, aes(x=trialDuration, y=changepoints, group=studyGroup)) + 
  geom_boxplot(aes(fill=studyGroup)) +
  facet_grid(targetCollected~studyGroup, margins=TRUE) +
...
```{r}
SpatMem_4 <- na.omit(dplyr::select(dataset_SpatMem_1, targetCollected, changepoints, straightness.index, trialDuration, interactions, trial_count, studyGroup))
