---
title: "Within-Participant Analysis"
author: "Zhiyi Sun"
date: "2023-12-28"
output:
  html_document:
...
# Plotting the trend
ggplot(changepoints_trend, aes(x = stage.true, y = mean_changepoints, 
...
model_changepoints_conf <- lmer(changepoints ~ stage.true + targetCollected + interactions 
                                + trial_count + (1|participant), data = data)
summary(model_changepoints_conf)

# Mixed-effects model including confounders (straightness.index) for trialDuration
model_trialDuration_si <- lmer(straightness.index ~ stage.true + changepoints + (1|participant), 
                               data = data)
summary(model_trialDuration_si)

# Mixed-effects model including confounders (changepoints) for trialDuration
model_trialDuration_cp <- lmer(trialDuration ~ stage.true + changepoints + (1|participant), 
                               data = data)
...
model_interaction <- lmer(trialDuration ~ stage.true * straightness.index + (1|participant), 
                          data = data)
...
```{r}
# Assuming interaction between 'stage.true' and 'straightness.index'
ggplot(data, aes(x = stage.true, y = trialDuration, color = straightness.index)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_minimal() +
  labs(title = "Interaction Effect of Stage and Straightness Index on Trial Duration",
       x = "Stage",
       y = "Trial Duration")
